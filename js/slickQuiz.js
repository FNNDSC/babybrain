        ...
        plugin.method = {
            ...
            setupQuiz: function() {
                ...
                for (i in questions) {
                    if (questions.hasOwnProperty(i)) {
                        ...
                        var questionHTML = $('<li class="question" id="question' + (count - 1) + '" data-callback="'+question.c+'"></li>');
                        ...
                        for (i in answers) {
                            if (answers.hasOwnProperty(i)) {
                                ...
                                // special case for custom HTML
                                if (answer.hasOwnProperty('html')) {
                                  
                                    answerContent = answer.html;
                        ...
                        if (plugin.config.backButtonText && plugin.config.backButtonText != '') {
                            questionHTML.append('<button class="button backToQuestion">' + plugin.config.backButtonText + '</button>');
                        }
                        ...
                        if (plugin.config.disableResponseMessaging || plugin.config.completionResponseMessaging) {
                            questionHTML.append('<button class="button nextQuestion checkAnswer">' + plugin.config.nextQuestionText + '</button>');
                        } else {
                            questionHTML.append('<button class="button nextQuestion">' + plugin.config.nextQuestionText + '</button>');
                            questionHTML.append('<button class="button checkAnswer">' + plugin.config.checkAnswerText + '</button>');
                        }
            ...
            startQuiz: function(startButton) {
                $(startButton).fadeOut(300, function(){
                    ...
                    var callback = firstQuestion.attr('data-callback');
                    if (callback != '') {
                      eval(callback);
                    }                    
            ...
            checkAnswer: function(checkButton) {
                ...
                // special case if a html widget is defined
                if (answers[0].hasOwnProperty('html')) {
                  
                    trueAnswers = [];
                    selectedAnswers.push(eval(answers[0].check));
                    trueAnswers.push(answers[0].correct);
            ...
            nextQuestion: function(nextButton) {
                ...
                // execute callback if it exists
                var callback = nextQuestion.attr('data-callback');
                if (callback != '') {
                  eval(callback);
                }
